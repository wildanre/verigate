 ...
 */
export function attachRequester(stream: StreamLike, client: RequesterClient, opts: RequesterOptions): Promise<Delivery> {
  const logger = opts.logger ?? console;
  let paying = false;

  return new Promise<Delivery>((resolve, reject) => {
    stream.on(EventType.OrderCreated, (e: Event) => {
      if (paying || !e.order_id) return;
      paying = true;
      client
        .payOrder(e.order_id)
...
export async function runRequester(client: RequesterClient, opts: RequesterOptions): Promise<Delivery> {
  ...
  const result = attachRequester(stream, client, opts);
